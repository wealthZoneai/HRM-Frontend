...
interface Props {
  ...
  onDelete?: (id: string | number) => void; // New prop for deletion
  initialData?: any | null; 
}

const AddPolicyModal: React.FC<Props> = ({ isOpen, onClose, onSubmit, onDelete, initialData }) => {
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar space-y-5">
          <div>
            ...
            <select
              ...
              className={`mt-2 w-full px-4 py-2.5 rounded-xl border ${errors.policy_type ? 'border-red-500' : 'border-gray-300'} bg-gray-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm`}
            >
          ...
          <div>
            ...
            <input
              ...
              className={`mt-2 w-full px-4 py-2.5 rounded-xl border ${errors.title ? 'border-red-500' : 'border-gray-300'} bg-gray-50 placeholder:opacity-50 focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm`}
            />
          ...
          <div>
              ...
              {form.content.map((c, idx) => (
                <div key={idx} className="flex gap-3 items-start group">
                  ...
                  <textarea
                    ...
                    className="flex-1 px-4 py-3 rounded-xl border border-gray-300 bg-gray-50 resize-none focus:bg-white focus:ring-2 focus:ring-blue-500 outline-none transition-all h-24 text-sm"
                  />
                  <button 
                    type="button" 
                    onClick={() => removeContent(idx)} 
                    className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all mt-1 opacity-0 group-hover:opacity-100"
                  >
                    <FiTrash2 size={18} />
                  </button>
                </div>
            ...
            <button
              type="button"
              onClick={addContent}
              className="mt-4 flex items-center gap-2 text-blue-600 font-semibold text-sm hover:text-indigo-700 transition-colors px-2 py-1"
            >
        ...
        <div className="p-5 border-t border-gray-100 flex justify-end gap-3 bg-gray-50 shrink-0">
          {initialData ? (
            ...
            <button 
              ...
              className="px-6 py-2.5 rounded-xl border border-red-200 text-red-600 bg-red-50 hover:bg-red-600 hover:text-white transition font-medium"
            >
              Delete Policy
            </button>
          ) : (
            // Cancel button shows only in Create Mode
            <button 
              type="button" 
              onClick={onClose} 
              className="px-6 py-2.5 rounded-xl border border-gray-300 text-gray-700 bg-white hover:bg-gray-100 transition font-medium"
            >
              Cancel
            </button>
          )}
          ...
          <button 
            ...
            className="px-8 py-2.5 rounded-xl bg-blue-600 text-white hover:bg-blue-700 shadow-lg shadow-blue-500/20 transition font-semibold"
          >
